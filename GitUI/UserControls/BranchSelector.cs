...
namespace GitUI.UserControls
{
    public partial class BranchSelector : GitModuleControl
    {
        ...
        public void Initialize(bool remote, IReadOnlyList<ObjectId>? containRevisions)
        {
            ...
            {
                return _localBranches ??= Module.GetRefs(GetRefsEnum.Branches).Select(b => b.Name).ToArray();
            }

            string[] GetRemoteBranches()
            {
                return _remoteBranches ??= Module.GetRefs(GetRefsEnum.Remotes).Select(b => b.Name).ToArray();
            }
