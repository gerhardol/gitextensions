...
namespace GitUI.BranchTreePanel
{
    internal sealed class RemoteBranchTree : BaseRefTree
    {
        ...
        protected override Nodes FillTree(IReadOnlyList<IGitRef> branches, CancellationToken token)
        {
            ...
            bool isRemoteRegex = !string.IsNullOrWhiteSpace(AppSettings.PrioritizedRemoteNames);
            enabledRemoteRepoNodes
                .OrderBy(node => node.FullPath)
                .OrderBy(node => isRemoteRegex && !Regex.IsMatch(node.FullPath, $"^({AppSettings.PrioritizedRemoteNames})$", RegexOptions.ExplicitCapture))
            ...
            if (disabledRemotes.Count > 0)
            {
                ...
                foreach (var remote in disabledRemotes.OrderBy(remote => remote.Name).OrderBy(node => isRemoteRegex && !Regex.IsMatch(node.Name, $"^({AppSettings.PrioritizedRemoteNames})$", RegexOptions.ExplicitCapture)))
                {
