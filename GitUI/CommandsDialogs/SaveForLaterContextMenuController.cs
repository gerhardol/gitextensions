...
namespace GitUI.CommandsDialogs
{
    public class SaveForLaterContextMenuController
    {
        ...
        /// <param name="isSecondRevision">Check if the first commit can be used as the first item</param>
        /// <returns>If the item can be used</returns>
        public bool ShouldEnableFirstRevDiff(FileStatusItem item, bool isSecondRevision)
        {
            // First item must be a git reference existing in the revision, i.e. other than work tree
            return item != null && !item.Item.IsSubmodule
                                && (isSecondRevision
                                    ? item.SecondRevision.ObjectId != ObjectId.WorkTreeId && !item.Item.IsDeleted
                                    : item.FirstRevision?.ObjectId != ObjectId.WorkTreeId && !item.Item.IsNew);
        }
        ...
        /// <param name="isSecondRevision">Check if the first commit can be used as the first item</param>
        /// <returns>If the item can be used</returns>
        public bool ShouldEnableSecondRevDiff(FileStatusItem item, bool isSecondRevision)
        {
