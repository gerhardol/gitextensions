        ...
        private void OpenMergeTool()
        {
            using (WaitCursorScope.Enter())
            {
                try
                {
                    ...
                    List<ConflictData> filesRemained = new List<ConflictData>();
                    ...
                    foreach (var conflictData in items)
                    {
                        ...
                        {
                            filesRemained.Insert(0, conflictData);
                        }
                    }
                    ...
                    _filesRemainedCount = filesRemained.Count;
                    ...
                    // For the remained files it should be asked for each file separately which one should be taken
                    // An empty string as a value of FooterCheckBox in TaskDialog hides the checkbox
                    ...
                    _solveMergeConflictApplyToAll = false;
                    _solveMergeConflictLastAction = ConflictResolutionPreference.None; // Reset the last selected action
                    foreach (var conflictData in filesRemained)
                    {
                        IncrementProgressBarValue();
                        ResolveItemConflict(conflictData);
                    }
