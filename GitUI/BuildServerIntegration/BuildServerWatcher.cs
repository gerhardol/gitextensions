...
namespace GitUI.BuildServerIntegration
{
    ...
    {
        private const int NumberOfPollSkipWhenNoBuildsAreRunning = 12;
        ...
        public async Task LaunchBuildServerInfoFetchOperationAsync()
        {
            ...
            // We are only interested by the 'OnComplete' which occurs:
            // * after 10s if a build is running
            // * after (NumberOfPollSkipWhenNoBuildsAreRunning * 10s) if no builds are running
            var delayObservable = Observable.Interval(TimeSpan.FromSeconds(10))
                .TakeWhile(a => !isBuildRunning && a < NumberOfPollSkipWhenNoBuildsAreRunning)
                .SkipWhile(_ => true).Select(_ => new BuildInfo());
