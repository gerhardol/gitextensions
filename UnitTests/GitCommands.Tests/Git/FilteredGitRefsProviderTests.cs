...
namespace GitCommandsTests.Git
{
    public sealed class FilteredGitRefsProviderTests
    {
        ...
        [Test]
        public void LazyGitRefsProviderTest()
        {
            var refs = new[]
            {
            ...
            FilteredGitRefsProvider lazyRefs = new(_module);

            lazyRefs.GetRefs(RefsFilter.NoFilter).Count.Should().Be(3);
            lazyRefs.GetRefs(RefsFilter.Remotes).Count.Should().Be(1);
            lazyRefs.GetRefs(RefsFilter.Heads).Count.Should().Be(2);
            lazyRefs.GetRefs(RefsFilter.Tags).Count.Should().Be(0);
            lazyRefs.GetRefs(RefsFilter.Remotes | RefsFilter.Heads).Count.Should().Be(3);
        }
    }
}
